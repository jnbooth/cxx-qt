...
mod global;
pub use global::{KeyboardModifier, MouseButton};
...
macro_rules! impl_qflag {
    ( $typeName:ty, $flagsName:ident, $typeId:literal ) => {
        pub type $flagsName = QFlags<$typeName>;

        unsafe impl QFlag for $typeName {
        ...
        impl BitOr for $typeName {
            type Output = QFlags<$typeName>;

            fn bitor(self, other: Self) -> Self::Output {
                QFlags::from(self) | other
            }
        }
    };
}

impl_qflag!(MouseButton, MouseButtons, "Qt::MouseButtons");
impl_qflag!(KeyboardModifier, KeyboardModifiers, "Qt::KeyboardModifiers");
